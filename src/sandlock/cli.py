...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.num_cpus:
        cli_kwargs["num_cpus"] = args.num_cpus
    ...
    if args.no_huge_pages:
        cli_kwargs["no_huge_pages"] = True
    if args.no_coredump:
...
def main() -> None:
    ...
    run_p.add_argument("--num-cpus", type=int, metavar="N",
                       help="Visible CPU count in /proc/cpuinfo")
    ...
    run_p.add_argument("--no-huge-pages", action="store_true",
                       help="Disable Transparent Huge Pages for deterministic page sizes")
