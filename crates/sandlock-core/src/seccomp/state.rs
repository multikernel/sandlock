...
pub struct NetworkState {
    ...
    /// Landlock domain, so the kernel rules never see it and this layer is
    /// the only allow enforcer there. `None` = unrestricted (deny-only
    /// mode or `'*'`); `Some(empty)` = the default deny-all.
