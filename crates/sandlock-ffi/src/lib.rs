...
pub unsafe extern "C" fn sandlock_sandbox_builder_max_memory(
    ...
    let builder = *Box::from_raw(b);
    let builder = match setter_arg(size, "max_memory") {
        Ok(s) => match ByteSize::parse(s) {
            Ok(v) => builder.max_memory(v),
            Err(e) => builder.reject_error(e),
        },
        Err(reason) => builder.reject(reason),
    };
    Box::into_raw(Box::new(builder))
...
pub unsafe extern "C" fn sandlock_sandbox_builder_max_disk(
    ...
    let builder = *Box::from_raw(b);
    let builder = match setter_arg(size, "max_disk") {
        Ok(s) => match ByteSize::parse(s) {
            Ok(v) => builder.max_disk(v),
            Err(e) => builder.reject_error(e),
        },
        Err(reason) => builder.reject(reason),
    };
...
pub unsafe extern "C" fn sandlock_sandbox_builder_time_start(
    ...
    let builder = *Box::from_raw(b);
    let builder = match setter_arg(timestamp, "time_start") {
        Ok(s) => match sandlock_core::profile::parse_time_start(s, "time_start") {
            Ok(t) => builder.time_start(t),
            Err(e) => builder.reject_error(e),
        },
        Err(reason) => builder.reject(reason),
    };
...
/// `b` must be a valid builder pointer.
#[no_mangle]
pub unsafe extern "C" fn sandlock_sandbox_builder_time_start_epoch(
    b: *mut SandboxBuilder,
    seconds: i64,
    nanoseconds: u32,
) -> *mut SandboxBuilder {
    ...
    let builder = *Box::from_raw(b);
    let builder =
        match sandlock_core::profile::time_start_from_epoch(seconds, nanoseconds, "time_start") {
            Ok(t) => builder.time_start(t),
            Err(e) => builder.reject_error(e),
        };
    Box::into_raw(Box::new(builder))
