//! sandlock-init: confined PID-1. Reads CONTROL_FD for Req messages and
//! fork-execs the workload (RunMain) and exec'd commands (RunExec). All children
//! inherit this process's seccomp filter and Landlock ruleset, so they share the
//! one supervisor. When the main workload exits, the container is done: kill the
//! process group and exit.
#[path = "proto.rs"]
mod proto;
mod fdrecv;

use proto::{Req, Resp, CONTROL_FD};
use std::ffi::CString;
...
fn send(fd: RawFd, resp: &Resp) {
    if let Ok(mut v) = serde_json::to_vec(resp) {
        v.push(b'\n');
        unsafe { libc::write(fd, v.as_ptr() as *const _, v.len()); }
    }
}

/// fork+exec `argv` with optional cwd/env and optional stdio fds (0,1,2).
/// Returns the child pid, or -1 on fork failure.
fn spawn(argv: &[String], env: &[(String, String)], cwd: &Option<String>, stdio: Option<[RawFd; 3]>) -> i32 {
    let pid = unsafe { libc::fork() };
    if pid != 0 { return pid; }
    // child
    if let Some(fds) = stdio {
        for (i, &fd) in fds.iter().enumerate() {
            unsafe { libc::dup2(fd, i as i32); }
        }
        for &fd in &fds {
            if fd > 2 { unsafe { libc::close(fd); } }
        }
    }
    if let Some(c) = cwd {
        if let Ok(cs) = CString::new(c.as_str()) {
            unsafe { libc::chdir(cs.as_ptr()); }
        }
    }
    for (k, v) in env {
        std::env::set_var(k, v);
...
fn wait_exit(pid: i32) -> (Option<i32>, Option<i32>) {
    ...
    if libc::WIFEXITED(status) { (Some(libc::WEXITSTATUS(status)), None) }
    else if libc::WIFSIGNALED(status) { (None, Some(libc::WTERMSIG(status))) }
    else { (None, None) }
}
...
fn main() {
    ...
    loop {
        ...
        if bytes.is_empty() { break; } // daemon closed the channel
        ...
        let req: Req = match serde_json::from_slice(trimmed) {
            Ok(r) => r,
            Err(e) => { send(ctl, &Resp::Err { msg: e.to_string() }); continue; }
        };
        match req {
            Req::RunMain { argv, env, cwd } => {
                let pid = spawn(&argv, &env, &cwd, None);
                if pid < 0 { send(ctl, &Resp::Err { msg: "fork failed".into() }); continue; }
                main_pid = pid;
            ...
            Req::RunExec { argv, env, cwd, detach } => {
                if fds.len() < 3 {
                    ...
                    send(ctl, &Resp::Err { msg: "exec needs 3 fds".into() }); continue;
                }
                let stdio = [fds[0], fds[1], fds[2]];
                let pid = spawn(&argv, &env, &cwd, Some(stdio));
                for &fd in &fds { unsafe { libc::close(fd); } } // parent drops its copies
                if pid < 0 { send(ctl, &Resp::Err { msg: "fork failed".into() }); continue; }
                send(ctl, &Resp::Started { pid });
                if !detach {
                    let ctl2 = ctl;
                    ...
                    std::thread::spawn(move || { let _ = wait_exit(pid); });
                }
            }
            Req::Shutdown => {
                if main_pid > 0 { unsafe { libc::killpg(libc::getpgrp(), libc::SIGKILL); } }
                break;
            }
        }
    }
