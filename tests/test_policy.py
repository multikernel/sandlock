...
class TestPolicy:
    ...
    def test_cpu_quota_percent(self):
        p = Policy(max_cpu="50%")
        assert p.cpu_quota_us() == 50000

    def test_cpu_quota_none(self):
        p = Policy()
        assert p.cpu_quota_us() is None

    def test_cpu_quota_absolute(self):
        p = Policy(max_cpu="25000")
        assert p.cpu_quota_us() == 25000
