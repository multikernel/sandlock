...
type Sandbox struct {
	...
	// When combined with NetAllow, denied destinations win.
	NetDeny []string
	...
	// deny these TCP ports (same port syntax). When combined with
	// NetAllowBind, denied ports win.
	NetDenyBind []string
	...
	MaxProcesses uint32   // peak concurrent process cap; 0 = unlimited
