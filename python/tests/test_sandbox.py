...
# The SDK forwards every readable path as written; nothing checks that one
# exists. A missing path fails late and anonymously, when the child installs
# the rule (`sandlock_create failed`, path not named), and under `chroot` the
# rule is skipped in silence instead. So the fixture filters rather than
# naming `/lib64` on a host (arm64, musl) that has none.
_PYTHON_READABLE = [
...
class TestNewPolicyFields:
    ...
    def test_time_start(self):
        ...
        # Freeze time to 2000-06-15. time_start takes epoch seconds or an RFC
        # 3339 stamp, and the SDK reads neither grammar: an aware datetime
        # converts to the former here, and the core resolves the latter.
