...
class Accepted:
    ...
    #: `None` for a value never compared on a live document (see `live`).
    renders: str | None
...
def _cargo_build() -> None:
    """Bring both artifacts up to date with the tree, once per session.

    Unconditional, rather than only when an artifact is missing. A tree
    carrying the artifacts of an earlier commit is the case this file cannot
    detect and must not compare against: a setter keeps its symbol name when
    its signature changes, so a stale library takes the `char *` this file
    passes and reads it as the integer it used to take. That compares the
    corpus against an ABI nobody is shipping, and says nothing about why.
    Cargo is a no-op on an already-built tree, so the cost of being sure is a
...
def _artifact(name: str, env_var: str) -> str:
    """Locate a build artifact, rebuilding the tree once to be sure of it.
