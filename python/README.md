...
# Every readable path is forwarded as written; the SDK no longer drops one
# that is not there, because whether a missing path is a portability detail or
# a typo is the caller's call. A missing path fails when the child installs
# the rule, without naming it. `/lib64` is absent on arm64 and on musl, so
...
| `max_memory` | `"256M"` | Memory limit: a size string or a count of bytes |
