...
| `max_processes`  | `processes`   | `int \| None`           | `None`  | Maximum number of **concurrent** processes in the sandbox (peak, not lifetime; threads do not count). Unlimited when unset, so a default sandbox does not contain a fork bomb; set it when running untrusted code. |
...
4. **HTTP host auto-expansion.** HTTP rules referencing concrete hosts
...
5. **TOCTOU and `policy_fn`.** Path strings are never exposed on
